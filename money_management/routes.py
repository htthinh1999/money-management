...
def process_gmail_data(gmail_data):
    ...
    app.logger.info(
        f"Received email from {email_address} with history ID: {history_id}"
    )
    ...
    histories = (
        gmail.users()
        .history()
        .list(userId="me", startHistoryId=CURRENT_HISTORY_ID)
        .execute()
    )
    set_current_history_id(history_id)

    app.logger.info(
        f"Processing histories: {histories} with history ID: {CURRENT_HISTORY_ID}"
    )
    ...
    for history in histories["history"]:
        ...
        for message in message_added:
            ...
            if (
                from_email != "VCBDigibank@info.vietcombank.com.vn"
                or "Biên lai" not in subject
            ):
                continue
            ...
            # process mail html
            cost = cost_convert(
                get_value_from_mail_html_with_i_tag(mail_html, "Amount")
            )
